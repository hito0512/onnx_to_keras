...
def onnx_to_keras(onnx_model, input_names,
                  input_shapes=None, name_policy=None, verbose=True, change_last=False):
    """
    ...
    for onnx_w in onnx_weights:
        ...
        logger.success('Found weight {0} with shape {1}.'.format(onnx_extracted_weights_name, weights[onnx_extracted_weights_name].shape))
    ...
    for i, input_name in enumerate(input_names):
        for onnx_i in onnx_inputs:
            if onnx_i.name == input_name:
                ...
                logger.success('Found input {0} with shape {1}'.format(input_name, input_shape))
    ...
    for node_index, node in enumerate(onnx_nodes):
        ...
        node_params['change_last'] = change_last
        ...
        logger.debug('node_name: %s --> [ %s ] '% ( node.name, node_name))
        ...
        for i, node_input in enumerate(node.input):
            ...
            if node_input not in layers:
                logger.warning('The input not found in layers / model inputs.')

                if node_input in weights:
                    logger.success('Found in weights, add as a numpy constant.')
                    layers[node_input] = weights[node_input]
                else:
                    raise AttributeError('Current node is not in weights / model inputs / layers.')
        else:
            logger.success('... found all, continue')
        ...
        try:
            logger.info('%s Layer Output --> %s' %(node.name, str(layers[keras_names].shape)))
        except KeyError:
            pass
