...
def convert_conv(node, params, layers, lambda_func, node_name, keras_name):
    ...
    if len(node.input) == 3:
        ...
        # Has bias
        has_bias = True
        W = ensure_numpy_type(layers[node.input[1]], params['change_last'], weight=True)
    ...
    #* @Hito: 
        ...
        if padding:
            logger.debug('Paddings exist, add ZeroPadding layer')
            padding_name = keras_name + '_pad'
            if params['change_last']:
                data_format='channels_last'
            else: 
                data_format='channels_first'
            padding_layer = keras.layers.ZeroPadding2D(padding=padding,name=padding_name,data_format=data_format)
            layers[padding_name] = input_0 = padding_layer(input_0)
